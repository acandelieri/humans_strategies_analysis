...
analysis_1 <- aggregate( collected$perc.len, by=list(collected$user,collected$tf,collected$uq), "sum" )
colnames( analysis_1) <- c( "user", "tf", "uq", "perc.length" )  

curr.mar <- par("mar")
par( mfrow=c(3,4) )
par( mar=c(4.1,4.6,2.1,1.1) )
for( tf in tfs ) {
  num.s <- analysis_1[which(analysis_1$tf==tf & analysis_1$uq=="sd"),]$perc.len
  num.h <- analysis_1[which(analysis_1$tf==tf & analysis_1$uq=="h"),]$perc.len
  num.z <- analysis_1[which(analysis_1$tf==tf & analysis_1$uq=="z"),]$perc.len
  
  hist.s <- hist( num.s, breaks=seq(0,100,by=10), plot=F )
  hist.h <- hist( num.h, breaks=seq(0,100,by=10), plot=F )
  hist.z <- hist( num.z, breaks=seq(0,100,by=10), plot=F )
  ...
  barplot( rbind(hist.s$counts,hist.h$counts,hist.z$counts),
           ...
           xlab="Pareto decisions [%]", names.arg=seq(10,100,by=10), 
...
par( mar=c(4.1,4.6,2.1,1.1) )
for( user in users ) {
  num.s <- analysis_1[which(analysis_1$user==user & analysis_1$uq=="sd"),]$perc.len
  num.h <- analysis_1[which(analysis_1$user==user & analysis_1$uq=="h"),]$perc.len
  num.z <- analysis_1[which(analysis_1$user==user & analysis_1$uq=="z"),]$perc.len
